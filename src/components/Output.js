...
export const Output = ({ textColor, backgroundColor }) => {
  ...
  return (
    <div className="outputContainer" style={{ background: backgroundColor }}>
      <h1 className="outputTitle" style={{ color: textColor }}>{quotes[number].title}</h1>
      <p className="outputText" style={{ color: textColor }}>{quotes[number].text}</p>
      <p className="outputAuthor" style={{ color: textColor }}>{quotes[number].author}</p>
    </div>
  );
}